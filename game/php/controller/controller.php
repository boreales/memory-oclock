    ...
    if(isset($_POST['function']) && isset($_POST['arguments'])){
        ...
        if($_POST['function'] === 'setScore'){
            ...
            if(isValidTimeStamp($time)){ //On vérifie si le temps est bien de type timestamp
                ...
                $score = new Score($time); //On crée un nouvel objet Score
                if(setScore($score)){ //On enregistre le score dans la base de données via notre fonction setScore du fichier functions.php
